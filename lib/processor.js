	...
	proto._spawnSox = function(args, options, processCB, endCB) {
		...
		utils.which('sox', function(err, soxPath) {
			...
			soxProc.on('exit', function(code, signal) {
				processExited = true;
				console.log("Soxproc exited with code "+ code + " and signal " + signal);
				if (signal) {
				...
				function processCB(soxProc) {
					...
					if (inputStream) {
						...
						inputStream.source.on('end', function() {
							console.log("\nInput stream ended.... bytes read: ", data_counter);
						});

						var waitTime = 2;
						var passThroughStream = through(
							function write(data){
								// console.log("Passing through data:", data.length);
								var queue_the_data = function(self, data) {
									return function() {
										// console.log("Queueing data", data.length);
										data_counter+= data.length;
										self.queue(data);
									};
								};
								setTimeout(queue_the_data(this, data), waitTime);
							}, function end(){
								var self = this;
								setTimeout(function() {
									console.log("Queueing null");
									console.log("Total data queued:", data_counter);
									self.queue(null);
								}, waitTime);
								console.log("End of pass through");
							});

						// inputStream.source.resume(); // opens the flow of data
						inputStream.source
